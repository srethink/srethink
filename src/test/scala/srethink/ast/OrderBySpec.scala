...
class OrderBySpec extends RethinkSpec with WithData {
  ...
  test("order the elements") {
    testQuery[Seq[Book]](b1, b2, b3)(
      books.orderBy(r.desc("id"), r.asc("seq")).limit(1)
    )(_.head.id == Some("3"))
  }
  test("order by index") {
    testQuery[Seq[Book]](b1, b2, b3)(books.orderByIndex(r.desc("sequence")))(
      _.head.id == Some("1")
    )
  }
}
