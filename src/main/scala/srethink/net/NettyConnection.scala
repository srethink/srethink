...
class NettyConnection(val config: NettyRethinkConfig) extends Connection {
  ...
  def connect() = {
    ...
    val channelFuture = bootstrap().connect(
      new java.net.InetSocketAddress(config.hostname, config.port)
    )
  ...
  private def bootstrap() = {
    ...
    val bufferFactory = new HeapChannelBufferFactory(
  ...
  class RethinkHandler extends SimpleChannelUpstreamHandler {
    ...
    override def messageReceived(
      ...
      e.getMessage match {
        ...
        case response: Response =>
          responseMap
            .get(response.token)
            .foreach(_.tryComplete(Success(response)))
    ...
    private def prepareQuery(ctx: ChannelHandlerContext) = {
      ...
      pipeline.replace(
        frameDecoderName,
        frameDecoderName,
        new LengthFieldBasedFrameDecoder(8 * 1024 * 1024, 8, 4)
      )
      pipeline.replace(
        messageDecoderName,
        messageDecoderName,
        new ResponseDecoder(charset)
      )
    ...
    private def prepareHandshake(ctx: ChannelHandlerContext) = {
      ...
      pipeline.addFirst(
        frameDecoderName,
        new DelimiterBasedFrameDecoder(1024, Delimiters.nulDelimiter: _*)
      )
    }
